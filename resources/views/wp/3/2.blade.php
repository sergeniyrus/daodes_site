  <h2 class="pros-cons">{{ __('wp/3/2.title') }}</h2>
  <p>{{ __('wp/3/2.paragraph1') }}</p>
  <ul>
      @foreach (__('wp/3/2.features_list') as $item)
          @php
  ...
  <h3>{{ __('wp/3/2.token_issuance_title') }}</h3>
  <p>{{ __('wp/3/2.token_issuance_paragraph') }}</p>
  <ol>
      @foreach (__('wp/3/2.token_issuance_list') as $item)
          @php
              $parts = explode(':', $item, 2); // Разделяем строку на 2 части
              $title = $parts[0]; // Первая часть (до ":")
              $description = isset($parts[1]) ? $parts[1] : ''; // Вторая часть (после ":"), если есть
          ...
          @if ($loop->first) <!-- Если это первый элемент, добавляем вложенный список -->
              <ul>
                  @foreach (__('wp/3/2.sub_list') as $subItem)
                      @php
