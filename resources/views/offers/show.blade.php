...
@section('main')
    ...
    <div class="container my-5">
        ...
        <div class="comment">
            <div class="z_com">
                <h4>Discussions: {{ $commentCount }}</h4>
            </div>

            @if ($commentCount == 0)
                <div class="eror_com">Be the first to share your opinion.</div>
            @else
                @foreach ($comments as $comment)
                    <div class="post_com">
                        <div class="name_com">
            ...
            @auth
                <div class="form_com">
                    <form name="comment" action="{{ route('comments.offers') }}" method="post">
                        @csrf
                        <fieldset>
                            <legend class="z_com"> Write a Message </legend>
                            <textarea id="editor" name="text" cols="80" rows="10" placeholder="Your opinion ..."></textarea>
                            <input type="hidden" name="offer_id" value="{{ $offers->id }}" />
                        </fieldset><br>
                        <div style="text-align: center">
                            <button type="submit" class="blue_btn">
            ...
            @else
                <div class="login_prompt">
                    <p>To leave a comment, <a href="{{ route('login') }}" class="login-link">log in</a>.</p>
                </div>
            @endauth
        </div>
