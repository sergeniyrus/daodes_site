...
@section('title_page', __('category.edit_category_title')) <!-- Перевод заголовка страницы -->
...
@section('main')
    ...
    <style>
        .container {
            padding: 20px;
            /* Увеличил отступы для более комфортного восприятия */
            margin: 0 auto;
            max-width: 600px;
            ...
            /* Золотая граница для контейнера */
            border-radius: 20px;
            /* Скругленные углы */
            color: #f8f9fa;
            /* Цвет текста */
        ...
        .form-group {
            margin-bottom: 20px;
            text-align: center;
            /* Центрирование содержимого внутри формы */
        }
    ...
    <div class="container my-5">
        <h1>{{ __('category.edit_category_title') }}</h1> <!-- Перевод заголовка -->
        ...
        <form action="{{ route('offerscategories.update', $category->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="name">{{ __('category.category_name_label') }}</label> <!-- Перевод метки -->
                <input type="text" name="name" id="name" class="input_dark"
